...
impl HookEchoApp {
    ...
    pub(super) fn dock_inspector(&mut self, ctx: &egui::Context) {
        ...
        egui::Area::new(egui::Id::new("dock_inspector"))
            .constrain_to(map_rect)
            .movable(false)
            .anchor(
                egui::Align2::RIGHT_TOP,
                egui::vec2(
                    -(ctx.content_rect().right() - map_rect.right() + SCALE_CLEAR),
                    map_rect.top() + 12.0,
                ),
            )
            .show(ctx, |ui| {
                ws::style_scope(ui, &t);
                ws::card_frame(&t).show(ui, |ui| {
                    ui.set_width(CARD_W);
                    ui.spacing_mut().item_spacing.y = 2.0;
                    if ws::panel_header(ui, &t, ph::INFO, "Inspector", None)
                        == ws::HeaderAction::Close
                    {
                        close = true;
                    }
                    egui::Frame::NONE
                        ...
                        .show(ui, |ui| {
                            ...
                            match (&probe, &value_line) {
                                (Some(p), Some((val, color))) => {
                                    ...
                                    for (k, v) in
                                        probe_rows(cursor_readout((p.lon, p.lat), radar, metric))
                                    {
                                        ws::kv(ui, &t, k, &v, None);
                                    }
                                    ws::kv(
        ...
        if close {
            self.dock.inspector_open = false;
        }
        if pin {
